...
### 1. Redis를 활용한 다각적인 성능 최적화
RDBMS의 부하를 줄이고 사용자 응답 속도를 향상시키기 위해, 데이터의 특성에 따라 Redis를 전략적으로 활용했습니다.
- **인증 성능 개선 (Refresh Token)**: 빈번한 인증 I/O 작업을 DB에서 Redis로 이전하고, TTL을 이용해 만료된 토큰을 자동 관리함으로써 인증 성능과 운영 효율성을 높였습니다.
- **읽기 성능 최적화 (Look-Aside Caching)**: 반복 조회되는 스터디/게시글 상세 정보에 Spring Cache(`@Cacheable`, `@CacheEvict`)를 적용하여 DB 부하를 줄이고 응답 속도를 개선했습니다. 데이터 변경 시 캐시를 동기화하는 전략을 구현하여 데이터 정합성을 유지했습니다.
- **랭킹보드 캐싱 (Sorted Set)**: 집계 연산이 많은 '핫 게시물' 목록을 `@Scheduled`를 통해 주기적으로 미리 계산하고, Redis의 **Sorted Set(ZSET)**에 저장하여 DB 접근 없이 매우 빠른 랭킹 조회가 가능하도록 설계했습니다.
- **쓰기 성능 최적화 (Write-Back 유사 패턴)**: 모든 조회마다 발생하던 조회수 `UPDATE` 쿼리를 Redis의 `INCR` 명령으로 대체하여 DB 쓰기 부하를 획기적으로 줄였습니다. Redis에 누적된 조회수는 스케줄러를 통해 주기적으로 DB에 동기화됩니다.

### 2. 이벤트 기반 비동기 아키텍처 설계
사용자의 활동에 따른 부가적인 작업(포인트/뱃지 부여, 활동 피드/알림 생성)을 비동기적으로 처리하기 위해 Spring의 **`ApplicationEventPublisher`**를 도입했습니다.
- `@TransactionalEventListener(phase = AFTER_COMMIT)`와 `@Async`를 조합하여, 메인 트랜잭션이 성공적으로 커밋된 후에만 별도의 스레드에서 부가 작업을 실행하도록 보장했습니다.
- 이벤트를 발행하는 쪽에서는 필요한 모든 데이터(ID, 제목 등)를 이벤트 객체에 담아 전달하고, 리스너는 DB 재조회 없이 전달받은 데이터만으로 로직을 처리하도록 설계하여 **트랜잭션 타이밍 문제와 데이터 불일치 가능성을 원천적으로 차단**했습니다.
...
- **목적별 프로토콜 선택**: 양방향 통신이 필수적인 채팅 기능에는 STOMP를, 서버에서 클라이언트로의 단방향 데이터 전송만 필요한 실시간 알림에는 더 가벼운 SSE를 채택하여 기술을 적재적소에 활용했습니다.
- **동시 접속자 수 관리**: `Redis Set`을 이용하여 각 페이지(채널)별 접속자를 실시간으로 집계하고, 사용자의 입장/퇴장 뿐만 아니라 비정상적인 **WebSocket 연결 종료(`SessionDisconnectEvent`)**까지 처리하여 정확한 접속자 수를 모든 클라이언트에게 브로드캐스팅하는 기능을 구현했습니다.
