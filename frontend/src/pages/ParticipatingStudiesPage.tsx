// src/pages/ParticipatingStudiesPage.tsx (새 파일)
...
const ParticipatingStudiesPage: React.FC = () => {
    ...
    const fetchParticipatingStudies = useCallback(async (page: number, isNewFetch: boolean) => {
        ...
        try {
            const response = await api.get<{ content: StudyGroupResponse[], last: boolean }>(
                '/api/users/me/participating-studies',
                { params: { page, size: 9 } }
            );
            ...
            setError(err.response?.data?.message || '참여중인 스터디를 불러오는데 실패했습니다.');
        } finally {
            setIsLoading(false);
    ...
    useEffect(() => {
        ...
        if (!isLoggedIn) {
            navigate('/login', { replace: true, state: { from: '/mypage/participating-studies' } });
            return;
        }
        fetchParticipatingStudies(0, true);
    }, [authLoading, isLoggedIn, navigate, fetchParticipatingStudies]);

    const handleLoadMore = () => {
        if (hasMore && !isLoadingMore) {
            fetchParticipatingStudies(currentPage + 1, false);
        }
    };
...
export default ParticipatingStudiesPage;